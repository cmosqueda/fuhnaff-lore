...
const WIKI = process.env.LORE_WIKI_BASE_URL ?? "https://freddy-fazbears-pizza.fandom.com";
const USER_AGENT = process.env.LORE_USER_AGENT ?? "fnaf-cards-lore-fetch/1.0 (github.com/cmosqueda/fuhnaff-lore)";
...
function normalizeGame(debutText) {
  ...
  if (debutText === "Five Nights at Freddy's") return "FNaF 1";
  const numbered = debutText.match(/^Five Nights at Freddy's\s+(\d+)$/);
  if (numbered) return `FNaF ${numbered[1]}`;
  // Subtitled entries ("Five Nights at Freddy's: Sister Location") read
  // better as just the subtitle, matching how appearsIn labels them.
  const subtitled = debutText.match(/^Five Nights at Freddy's:\s*(.+)$/);
  if (subtitled) return subtitled[1];
  return debutText;
}
...
// Runs `fn` over `items` with at most `limit` in flight at once. With the
// character/place scope now spanning 10 games, a candidate list can run to
// several hundred pages — firing a fetch (or three) per page all at once
// exhausted the connection pool and crashed with a timeout. Fandom's own
// scraping guidance also asks for exactly this: don't hammer it with
...
async function discoverPages({ categories, exclude, excludePatterns, requiredFields, withAppearsIn }) {
  ...
  const pages = await mapWithConcurrency(titles, FETCH_CONCURRENCY, async (title) => {
...
export async function fetchLoreData(config) {
  const [characters, places, movies, books, games] = await Promise.all([
    ...
    discoverBooks(
      ...
      config.bookOverrides ?? {},
      config.bookExclude ?? [],
      config.bookExcludePatterns,
    ),
    discoverGames(
      config.gameCategories,
      config.gameOverrides ?? {},
      config.gameExclude ?? [],
      config.gameExcludePatterns,
    ),
  ]);
